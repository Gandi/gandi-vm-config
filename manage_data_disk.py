        ...
        for line in open(config_file, "r"):
            ...
            if match is not None:
                ...
                if defined_value != "":
                    ...
                    syslog.syslog(
                        "%s is empty. Using defaults : %s." % (label, default_value)
                    )
    except IOError:
...
def run_command(label, cmd):
    ...
    command = subprocess.Popen(
        cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE, stdin=None, shell=True
    )
    ...
    if ret is not None:
        syslog.syslog(
            "%s [%s]: %s stderr=%s"
            % (label, cmd, output.decode("utf-8"), error.decode("utf-8"))
        )
    else:
...
def pre_add(device, fs_type, mountpoint):
    ...
    if not fs_type == "ext3" and not fs_type == "ext4" and not fs_type == "ext2":
        syslog.syslog(
            "%s: does not contain an ext2/3/4 filesystem. \
            We simply mount it as is."
            % device
        )
        return
    ...
    if re.search("^ext[2-4]", fs_type):
        ...
        if not os.path.exists(resize_bin):
            ...
            if not os.path.exists(resize_bin):
                syslog.syslog(
                    "%s: resize binary is not present " % device
                    + "(either resize2fs or resize4fs)."
                )
                return

        run_command("auto-resize", "%s %s %s" % (resize_bin, resize_opts, device))
    else:
...
def post_add(device, fs_type, mountpoint):
    ...
    run_command(
        "call shell hook in post add",
        "%s/post-disk-attach %s %s %s"
...
def pre_delete(device, mountpoint):
    ...
    run_command(
        "call shell hook in pre delete",
        "%s/pre-disk-detach %s %s" % (load_hook_dir(config_file), device, mountpoint),
    )
    return
...
def on_add(device, mountpoint):
    ...
    if os.environ.get("ID_FS_TYPE"):
        cmd = "mount -t %s -o %s %s %s" % (
            os.environ["ID_FS_TYPE"],
            mount_params,
            device,
            mountpoint,
        )
...
def remove_mountpoint(device, mountpoint):
    ...
    if not os.path.ismount(mountpoint):
        ...
        syslog.syslog("%s: warning, something is mounted on %s" % (device, mountpoint))
    ...
    if os.path.exists(os.path.join(mountpoint, ".gandi")):
        if os.path.exists(os.path.join(mountpoint, ".gandi", "services")):
            ...
            for service in services:
                ...
                if ret is not None:
                    syslog.syslog(
                        "stopping %s, output : %s and error : %s"
                        % (service, output.decode("utf-8"), error.decode("uft-8"))
                    )
...
def is_mounted(device):
    ...
    for line in open("/proc/mounts", "r"):
        ...
        if match is not None:
            ...
            syslog.syslog("%s: already mounted on the system." % device)
            sys.exit(0)
...
def get_mountpoint(device, rawdevice, disk_root):
    ...
    if "ID_FS_LABEL_SAFE" in os.environ and os.environ["ID_FS_LABEL_SAFE"]:
        ...
        if "ID_FS_LABEL_ENC" in os.environ and os.environ["ID_FS_LABEL_ENC"]:
            mountpoint = disk_root + "/" + os.environ["ID_FS_LABEL_ENC"]
            if debug:
                msg = "label from udev is"
                syslog.syslog(
                    "%s: %s is %s and mountpoint is %s"
            ...
            if ret is None and not re.compile("^$").match(output.decode("utf-8")):
                ...
                mountpoint = disk_root + "/" + os.path.basename(rawdevice)
            if debug:
                syslog.syslog(
                    "%s: label info from e2label, mountpoint is %s"
                    % (device, mountpoint)
                )

    if re.match("add", os.environ["ACTION"]) or re.match(
        "change", os.environ["ACTION"]
    ):
        if debug:
            syslog.syslog(
                "%s: looking for mountpoint %s in /proc/mounts." % (device, mountpoint)
            )
...
def detect_devicename(re_deviceline, pattern_id):
    ...
    for line in open("/proc/partitions", "r").readlines():
        ...
        if debug:
            syslog.syslog(
                "detect in /proc/partitions, line is %s, result %s"
                % (line, ", ".join(result.groups()))
            )
        device_name = result.group(pattern_id)
        device = "/dev/" + device_name
        if debug:
            syslog.syslog("manage_data_disk: correct name detected is %s" % device)
        ...
        if debug:
            syslog.syslog("partitions: %s, %s" % (device_label, ", ".join(partitions)))
...
def main():
    ...
    if debug:
        re_info = re.compile(
            "^(LANG|TERM|SHELL|SSH|LC_|LS_|HOME|MAIL|OLD|SHL|LOGNAME|USER).*$"
        )
    ...
    if (
        not os.environ.get("DEVNAME")
        and os.environ.get("DEVPATH")
        and re.match(pattern, os.environ["DEVPATH"])
    ):
        if debug:
        ...
        if debug:
            syslog.syslog("%s: %s:%s (major/minor)" % (rawdevice, major, minor))

        # as long as the device is connected, we can find the real name with
        # minor and major info
        re_deviceline = re.compile(
            "\s+(%s)\s+(%s)\s+.*((xv|s)d[a-z0-9]+)$" % (major, minor)
        )
    ...
    if os.environ.get("DEVNAME"):
        if debug:
            syslog.syslog(
                "manage_data_disk: using DEVNAME (%s)" % os.environ.get("DEVNAME")
            )
    ...
    try:
        if debug:
            syslog.syslog(
                "%s: get_mountpoint(), raw: %s and root: %s"
    ...
    if re.match("add", os.environ["ACTION"]) or re.match(
        ...
        if debug:
            syslog.syslog("%s: check before adding disk : is_mounted()" % device)
        is_mounted(device)
        ...
        if len(partitions.get(device_label, [])) == 1:
            if debug:
                syslog.syslog(
                    "%s: action is %s, calling pre_add()"
                    % (device, os.environ["ACTION"])
                )
            pre_add(device, fs_type, mountpoint)
        if debug:
            syslog.syslog(
                "%s: action is %s, calling on_add()" % (device, os.environ["ACTION"])
            )
    ...
    if re.match("offline", os.environ["ACTION"]) or (
        re.match("remove", os.environ["ACTION"])
        and re.match("gandi\.ne", os.environ["ID_VENDOR"])
    ):
        if debug:
            syslog.syslog(
                "%s: action is %s, calling on_delete()" % (device, os.environ["ACTION"])
            )
